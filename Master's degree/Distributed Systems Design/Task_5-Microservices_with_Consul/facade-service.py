...
service_id = consulver.service_register("facade-service", args.port)
consul_settings = consulver.get_settings("lab_settings")


def get_random_service(service_name):
    return consulver.get_address(service_name)
...
def sending_with_retry(data):
    ...
    try:
        logging_service = get_random_service("logging-service") + consul_settings["api-endpoints"]["log"]
...
def receiving_with_retry(log_endpoint, msg_endpoint):
    ...
    try:
        logging_service = get_random_service(log_endpoint) + consul_settings["api-endpoints"]["log"]
        logging_response = requests.get(logging_service, timeout=5)

        messages_service = get_random_service(msg_endpoint) + consul_settings["api-endpoints"]["msg"]
...
def request_handler():
    ...
    if request.method == "POST":
        ...
        try:
            msg_queue.put(msg)
            print(f"- Message Was Queued: '{msg}'")
...
if __name__ == "__main__":
    facade_ports = [8080, 8088]
    if args.port not in facade_ports:
        print(f"[ERROR] Port '{args.port}' is not in the"
              f" list of logging ports: {facade_ports}")
        sys.exit(1)
