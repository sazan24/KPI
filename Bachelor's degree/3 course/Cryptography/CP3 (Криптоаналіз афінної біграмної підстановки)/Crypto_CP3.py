...
def athenian_decryption(filename: str, mod: int = 961) -> 0:
    ...
    all_a = []
    length = len(most_frequent_bigrams)
    check = 0
                ...
                for index4 in range(length):
                    ...
                    for curr in range(len(possible_a)):
                        ...
                        print(f"a = {a}, b = {b}    |    (a,b) –→ ({a}, {b})\n"
                              f"Entropy: H = {round(entropy, 4)}")
                        print(str(decrypted_text_list_copy[:100]))
                        opt = input("\n☼ Does this look like valid text?\n  "
                                    "  Y/y –→ for writing text in file;\n  "
                                    "  N/n –→ to continue decrypting.\n"
                                    ">>> ")
                        if opt.upper() == "Y":
                            answer = open("./decrypted_text.txt", "w")
                            answer.write("".join(decrypted_text_list_copy))
                            answer.close()
                            print("[◘]► Text was written to the file ./decrypted_text.txt")
                            check = 1
                            break
                    if check:
                        break
...
cipher_text = input("\n☼ Enter relative path of file to decrypt:\n"
                    ">>> ")
